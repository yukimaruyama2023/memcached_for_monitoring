...
#define BIN_MAX_EXTLEN                                                         \
  20 // length of the _incr command is currently the longest.
...
#define SLAB_GLOBAL_PAGE_POOL                                                  \
  0 /* magic slab class for storing pages for reassignment */
...
#define ITEM_get_cas(i)                                                        \
  (((i)->it_flags & ITEM_CAS) ? (i)->data->cas : (uint64_t)0)
...
#define ITEM_ntotal(item)                                                      \
  (sizeof(struct _stritem) + (item)->nkey + 1 + (item)->nbytes +               \
   (((item)->it_flags & ITEM_CFLAGS) ? sizeof(client_flags_t) : 0) +           \
   (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))
 ...
 */
typedef void (*ADD_STAT)(const char *key, const uint16_t klen, const char *val,
...
// reorder members to gather metrics collected
typedef struct {
  uint64_t evicted;               // *
  uint64_t reclaimed;             // *
...
enum stop_reasons { NOT_STOP, GRACE_STOP, EXIT_NORMALLY };
...
struct stats {
  ...
  uint64_t
      slab_reassign_evictions_nomem;     /* valid items lost during slab move */
  ...
  uint64_t
      time_in_listen_disabled_us; /* elapsed time in microseconds while server
                                     unable to process new connections */
  ...
  uint64_t ssl_proto_errors; /* TLS failures during SSL_read() and SSL_write()
                                calls */
  uint64_t ssl_handshake_errors; /* TLS failures at accept/handshake time */
  uint64_t ssl_new_sessions;     /* successfully negotiated new (non-reused) TLS
                                    sessions */
...
struct settings {
  size_t maxbytes;
  int maxconns;
  int num_threads; /* number of worker (without dispatcher) libevent threads to
                      run */
  bool maxconns_fast; /* Whether or not to early close connections */
#ifdef TLS
  rel_time_t ssl_last_cert_refresh_time; /* time of the last server certificate
                                            refresh */
#endif
  // only above is collected
  ...
  volatile sig_atomic_t
      sig_hup;      /* a HUP signal was received but not yet handled */
  ...
  unsigned int
      logger_watcher_buf_size;  /* size of logger's per-watcher buffer */
  unsigned int logger_buf_size; /* size of per-thread logger buffer */
  unsigned int
      read_buf_mem_limit; /* total megabytes allowable for net buffers */
  ...
  unsigned int
      ext_drop_under; /* when fewer than this many pages, drop COLD items */
  unsigned int
      ext_max_sleep;   /* maximum sleep time for extstore bg threads, in us */
...
#endif
#ifdef TLS
  void *ssl_ctx;        /* holds the SSL server context which has the server
                           certificate */
  ...
  unsigned int
      ssl_wbuf_size; /* size of the write buffer used by ssl_sendmsg method */
...
struct io_queue_s {
  ...
  int count;       // ios to process before returning. only accessed by queue
                   // processor once submitted
...
typedef struct io_queue_cb_s {
  ...
  io_queue_stack_cb
      submit_cb; // callback given a full stack of pending IO's at once.
  int type;
} io_queue_cb_t;
...
typedef struct _mc_resp {
  ...
  item *
      item; /* item associated with this response object, with reference held */
  struct iovec
      iov[MC_RESP_IOVCOUNT]; /* built-in iovecs to simplify network code */
  ...
  uint16_t
      request_id; /* Incoming UDP request ID, if this is a UDP "connection" */
...
#define MAX_RESP_PER_BUNDLE                                                    \
  ((READ_BUFFER_SIZE - sizeof(mc_resp_bundle)) / sizeof(mc_resp))
...
struct conn {
  ...
  enum network_transport
      transport; /* what transport is used by this connection */
  ...
  LIBEVENT_THREAD
      *thread; /* Pointer to the thread object serving this connection */
...
enum delta_result_type do_add_delta(LIBEVENT_THREAD *t, const char *key,
                                    ...
                                    item **it_ret);
enum store_item_type do_store_item(item *item, int comm, LIBEVENT_THREAD *t,
                                   const uint32_t hv, int *nbytes,
                                   uint64_t *cas, const uint64_t cas_in,
                                   bool cas_stale);
...
item *item_alloc(const char *key, size_t nkey, client_flags_t flags,
                 rel_time_t exptime, int nbytes);
#define DO_UPDATE true
#define DONT_UPDATE false
item *item_get(const char *key, const size_t nkey, LIBEVENT_THREAD *t,
               const bool do_update);
item *item_get_locked(const char *key, const size_t nkey, LIBEVENT_THREAD *t,
...
enum store_item_type store_item(item *item, int comm, LIBEVENT_THREAD *t,
                                int *nbytes, uint64_t *cas,
                                const uint64_t cas_in, bool cas_stale);
...
#define EXPTIME_TO_POSITIVE_TIME(exptime)                                      \
  (exptime < 0) ? REALTIME_MAXDELTA + 1 : exptime
