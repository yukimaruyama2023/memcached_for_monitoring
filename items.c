...
unsigned int do_get_lru_size(uint32_t id) { return sizes[id]; }
...
item *do_item_alloc(const char *key, const size_t nkey,
                    const client_flags_t flags, const rel_time_t exptime,
                    const int nbytes) {
  uint8_t nsuffix;
  item *it = NULL;
  char suffix[40];
 ...
 */
bool item_size_ok(const size_t nkey, const client_flags_t flags,
                  const int nbytes) {
  char prefix[40];
  uint8_t nsuffix;
  if (nbytes < 2)
  ...
  *head = it;
  ...
  if (it->it_flags & ITEM_HDR) {
    sizes_bytes[it->slabs_clsid] +=
        (ITEM_ntotal(it) - it->nbytes) + sizeof(item_hdr);
  } else {
...
void fill_item_stats_automove(item_stats_automove *am) {
  ...
  for (n = 0; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
    ...
    if (!tails[i]) {
      cur->age = 0;
    } else if (tails[i]->nbytes == 0 && tails[i]->nkey == 0 &&
               tails[i]->it_flags == 1) {
...
void item_stats(ADD_STAT add_stats, void *c) {
  ...
  for (n = 0; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
    ...
    APPEND_NUM_FMT_STAT(fmt, n, "mem_requested", "%llu",
                        (unsigned long long)totals.mem_requested);
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv,
                  LIBEVENT_THREAD *t, const bool do_update) {
  ...
  LOGGER_LOG(t->l, LOG_FETCHERS, LOGGER_ITEM_GET, NULL, was_found, key, nkey,
             (it) ? it->nbytes : 0, (it) ? ITEM_clsid(it) : 0, t->cur_sfd);
...
static int lru_maintainer_juggle(const int slabs_clsid) {
  ...
  slabs_available_chunks(slabs_clsid, NULL, &chunks_perslab);
  if (settings.temp_lru) {
